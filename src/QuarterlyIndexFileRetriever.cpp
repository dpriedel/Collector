...
//
//    Description:  Implements a class which knows how to retrieve SEC quarterly
//    index files
//
//        Version:  1.0
...
//--------------------------------------------------------------------------------------
QuarterlyIndexFileRetriever::QuarterlyIndexFileRetriever(
    const std::string &host, const std::string &port, const fs::path &prefix)
...
fs::path QuarterlyIndexFileRetriever::HierarchicalCopyRemoteIndexFileTo(
    const fs::path &remote_file_name, const fs::path &local_directory_name,
    bool replace_files) {
  auto local_quarterly_index_file_name =
      this->MakeLocalIndexFilePath(local_directory_name, remote_file_name);

  if (!replace_files && fs::exists(local_quarterly_index_file_name)) {
    spdlog::info(
        catenate("Q: File exists and 'replace' is false: skipping download: ",
                 local_quarterly_index_file_name.filename().string()));
    return local_quarterly_index_file_name;
  }

  auto local_quarterly_index_file_directory =
      local_quarterly_index_file_name.parent_path();
  ...
  spdlog::info(catenate(
      "Q: Retrieved remote quarterly index file: ", remote_file_name.string(),
      " to: ", local_quarterly_index_file_name.string()));

  return local_quarterly_index_file_name;
} // -----  end of method QuarterlyIndexFileRetriever::CopyRemoteIndexFileTo
  // -----

fs::path QuarterlyIndexFileRetriever::MakeLocalIndexFilePath(
  ...
  std::string remote_index_name = boost::algorithm::replace_first_copy(
      remote_quarterly_index_file_name.string(),
...
std::vector<fs::path>
QuarterlyIndexFileRetriever::HierarchicalCopyIndexFilesForDateRangeTo(
  ...
  for (const auto &remote_file : remote_file_list) {
    results.push_back(HierarchicalCopyRemoteIndexFileTo(
        remote_file, local_directory_name, replace_files));
  }

  return results;
} // -----  end of method
  // QuarterlyIndexFileRetriever::CopyIndexFilesForDateRangeTo  -----
...
auto QuarterlyIndexFileRetriever::AddToCopyList(
  ...
  return [this, local_directory_name,
    ...
    fs::create_directories(local_quarterly_index_file_directory);
    return HTTPS_Downloader::copy_file_names(
        remote_file_name, std::move(local_quarterly_index_file_name));
  };
}

std::vector<fs::path> QuarterlyIndexFileRetriever::
    ConcurrentlyHierarchicalCopyIndexFilesForDateRangeTo(
        const std::vector<fs::path> &remote_file_list,
  ...
  if (remote_file_list.size() < max_at_a_time) {
    return HierarchicalCopyIndexFilesForDateRangeTo(
        remote_file_list, local_directory_name, replace_files);
  }
  ...
  spdlog::info(catenate("Q: Downloaded: ", success_counter,
                        " Skipped: ", skipped_files_counter,
                        " Errors: ", error_counter, " quarterly index files."));
