...
FinancialStatementsAndNotes_gen::FinancialStatementsAndNotes_gen(std::chrono::year_month_day start_date,
...
void FinancialStatementsAndNotes_gen::format_current_value()
{
    if (!monthly_mode_)
    {
        ...
        current_value_.second =
            std::format("{}_{}", current_date_.year().operator int(), current_date_.month().operator unsigned int());
    }
    else
    {
        current_value_.first = std::format("{}_{:02}_notes.zip", current_date_.year().operator int(),
                                           current_date_.month().operator unsigned int());
        current_value_.second =
            std::format("{}_{:02}", current_date_.year().operator int(), current_date_.month().operator unsigned int());
    }
...
// Description:  constructor
//--------------------------------------------------------------------------------------
FinancialStatementsAndNotes::FinancialStatementsAndNotes(std::chrono::year_month_day start_date,
                                                         std::chrono::year_month_day end_date)
...
void FinancialStatementsAndNotes::download_files(const std::string &server_name, const std::string &port,
                                                 ...
                                                 const fs::path &download_destination_files, bool replace_files)
{
    ...
    for (const auto &[file, directory] : *this)
    {
        ...
        fs::path destination_file_directory = download_destination_files / directory;
        ...
        BOOST_ASSERT_MSG(
            ...
            std::format("Unable to create/find destination_file_directory: {}", destination_file_directory).c_str());
        ...
        if (fs::exists(destination_zip_file) && replace_files == false)
        {
            ...
            spdlog::info(
                std::format("N: File: {} exists and 'replace' is false. Skipping download.", destination_zip_file));
            continue;
        }
        ...
        try
        {
            ...
            process proc(ctx.get_executor(), // <1>
                         "/usr/bin/7z",      // <2>
                         {"x"s, "-aoa", "-bd", "-bb0", "-bsp0", "-bso0", "-o"s + destination_file_directory.string(),
                          destination_zip_file.string()} // <3>
            );                                           // <4>
            auto result = proc.wait();
            if (result != 0)
            {
                throw std::runtime_error(std::format("Unzipping file: {} failed with return code: {}",
    ...
    spdlog::info(std::format("N: Downloaded: {}. Skipped: {}. Errors: {}. out of {} possible files.",
