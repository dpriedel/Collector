...
fs::path DailyIndexFileRetriever::FindRemoteIndexFileNameNearestDate(
  ...
  auto pos =
      std::find_if(directory_list.crbegin(), directory_list.crend(),
                   [&looking_for](const auto &x) {
...
DailyIndexFileRetriever::FindRemoteIndexFileNamesForDateRange(
  ...
  auto is_between{[&looking_for_start,
                   &looking_for_end](const std::string &elem) {
  ...
  for (const auto &remote_directory : remote_directory_list) {
    ...
    std::copy_if(directory_list.crbegin(), directory_list.crend(),
                 std::back_inserter(found_files), is_between);

    // last thing...prepend directory name

    std::transform(found_files.cbegin(), found_files.cend(),
...
DailyIndexFileRetriever::GetRemoteIndexList(const fs::path &remote_directory) {
...
fs::path DailyIndexFileRetriever::CopyRemoteIndexFileTo(
  ...
  if (!replace_files && fs::exists(local_daily_index_file_name)) {
    spdlog::info(
        catenate("D: File exists and 'replace' is false: skipping download: ",
  ...
  HTTPS_Downloader the_server(host_, port_);
  the_server.DownloadFile(remote_daily_index_file_name,
                          local_daily_index_file_name);

  spdlog::info(catenate("D: Retrieved remote daily index file: ",
                        remote_daily_index_file_name.string(),
...
fs::path DailyIndexFileRetriever::HierarchicalCopyRemoteIndexFileTo(
    const fs::path &remote_daily_index_file_name,
    const fs::path &local_directory_prefix, bool replace_files) {
  auto local_daily_index_file_name = MakeLocalIndexFilePath(
      local_directory_prefix, remote_daily_index_file_name);
  ...
  auto local_daily_index_file_directory =
      local_daily_index_file_name.parent_path();
...
std::vector<fs::path> DailyIndexFileRetriever::CopyIndexFilesForDateRangeTo(
  ...
  for (const auto &remote_daily_index_file_name : remote_file_list) {
    auto local_file = CopyRemoteIndexFileTo(
        remote_daily_index_file_name, local_directory_name, replace_files);
...
auto DailyIndexFileRetriever::AddToCopyList(
  ...
  return [local_directory_name, replace_files](const auto &remote_file_name) {
    ...
    return HTTPS_Downloader::copy_file_names(
        remote_file_name, std::move(local_daily_index_file_name));
  };
}

std::vector<fs::path>
DailyIndexFileRetriever::ConcurrentlyCopyIndexFilesForDateRangeTo(
    const std::vector<fs::path> &remote_file_list,
    const fs::path &local_directory_name, int max_at_a_time,
    bool replace_files) {
  if (remote_file_list.size() < max_at_a_time) {
    return CopyIndexFilesForDateRangeTo(remote_file_list, local_directory_name,
                                        replace_files);
  }
  ...
  std::transform(std::begin(remote_file_list), std::end(remote_file_list),
  ...
  spdlog::info(catenate("D: Downloaded: ", success_counter,
...
std::vector<fs::path>
DailyIndexFileRetriever::HierarchicalCopyIndexFilesForDateRangeTo(
    const std::vector<fs::path> &remote_file_list,
    const fs::path &local_directory_prefix, bool replace_files) {
  std::vector<fs::path> results;

  for (const auto &remote_daily_index_file_name : remote_file_list) {
    auto local_file = HierarchicalCopyRemoteIndexFileTo(
        remote_daily_index_file_name, local_directory_prefix, replace_files);
    results.push_back(std::move(local_file));
  }
  return results;
} // -----  end of method DailyIndexFileRetriever::CopyIndexFilesForDateRangeTo
...
auto DailyIndexFileRetriever::AddToConcurrentCopyList(
  ...
  return [this, local_directory_prefix,
    ...
    auto local_daily_index_file_name =
        this->MakeLocalIndexFilePath(local_directory_prefix, remote_file_name);
    local_daily_index_file_name /= remote_file_name.filename();
...
DailyIndexFileRetriever::ConcurrentlyHierarchicalCopyIndexFilesForDateRangeTo(
    ...
    bool replace_files) {
  if (remote_file_list.size() < max_at_a_time) {
    return HierarchicalCopyIndexFilesForDateRangeTo(
        remote_file_list, local_directory_prefix, replace_files);
  }
  ...
  std::transform(
      std::begin(remote_file_list), std::end(remote_file_list),
      std::back_inserter(concurrent_copy_list),
...
fs::path DailyIndexFileRetriever::MakeLocalIndexFilePath(
    const fs::path &local_prefix,
    const fs::path &remote_daily_index_file_name) {
  ...
  std::string remote_index_name = boost::algorithm::replace_first_copy(
      remote_daily_index_file_name.string(), remote_directory_prefix_.string(),
      "");
