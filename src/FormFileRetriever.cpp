...
auto FormFileRetriever::ExtractFileName(int& found_a_form)
{
    return [&found_a_form] (const auto& e)
    {
        ...
        BOOST_ASSERT_MSG(pos != std::string::npos, "Badly formed index file record.\n");
		...
		const fs::path& local_index_file_name, const TickerConverter::TickerCIKMap& ticker_map)
{
	...
	BOOST_ASSERT_MSG(index_file, catenate("Unable to open index file: ", local_index_file_name.string()).c_str());
	...
	for (auto& the_form : forms_list)
	{
		...
		auto list_begin = std::lower_bound(form_itor, itor_end, the_form,
            ...
            { return the_line.compare(0, the_form.size(), the_form.data(), the_form.size()) < 0; } );
		auto list_end = std::upper_bound(list_begin, itor_end, the_form,
			[] (const auto& the_form, const auto& the_line)
            { return the_form.compare(0, the_form.size(), the_line.data(), the_form.size()) < 0; } );

		if (list_begin == list_end)
        {
            // found no lines in index file for our form type.

			spdlog::debug(catenate("F: Found ", found_a_form, " files for form: ", the_form));
			continue;
        }
	...
	spdlog::debug(catenate("F: Found a total of ", grand_total, " files for specified forms."));
	return results;
