...
public:
    ...
    [[nodiscard]] std::pair<std::chrono::year_month_day, std::chrono::year_month_day> GetActualDateRange() const
    {
    ...
    fs::path FindRemoteIndexFileNameNearestDate(std::chrono::year_month_day aDate);
    ...
    fs::path HierarchicalCopyRemoteIndexFileTo(const fs::path &remote_daily_index_file_name,
                                               const fs::path &local_directory_prefix, bool replace_files = false);

    //	This method treats the date range as a closed interval.

    std::vector<fs::path> FindRemoteIndexFileNamesForDateRange(std::chrono::year_month_day start_date,
    ...
    std::vector<fs::path> CopyIndexFilesForDateRangeTo(const std::vector<fs::path> &remote_file_list,
                                                       const fs::path &local_directory_name,
                                                       bool replace_files = false);
    std::vector<fs::path> ConcurrentlyCopyIndexFilesForDateRangeTo(const std::vector<fs::path> &remote_file_list,
                                                                   const fs::path &local_directory_name,
