  ...
  if (!log_file_path_name_.empty()) {
    ...
    if (!logger_) {
      ...
      logger_ = spdlog::basic_logger_mt<spdlog::async_factory>(
          logger_name, log_file_path_name_.c_str());
...
void CollectorApp::SetupProgramOptions() {
  ...
  mNewOptions->add_options()("help,h", "produce help message")(
      ...
      "directory index files are downloaded to.")(
      "form-dir", po::value<fs::path>(&this->local_form_file_directory_),
      ...
      "Default is 'daily'.")(
      "form", po::value<std::string>(&this->form_)->default_value("10-Q"),
      "name of form type[s] we are downloading. Default is '10-Q'.")(
      "ticker", po::value<std::string>(&this->ticker_),
      "ticker[s] to lookup and filter form downloads.")(
      "log-path", po::value<fs::path>(&this->log_file_path_name_),
      "path name for log file")(
      "ticker-cache", po::value<fs::path>(&this->ticker_cache_file_name_),
      ...
      "-1 means no limit.")(
      "log-level,l",
      po::value<std::string>(&this->logging_level_)
          ->default_value("information"),
      "logging level. Must be 'none|error|information|debug'. Default is "
      "'information'.")(
      "concurrent,k", po::value<int>(&this->max_at_a_time_)->default_value(10),
...
bool CollectorApp::CheckArgs() {
  BOOST_ASSERT_MSG(mode_ == "daily" || mode_ == "quarterly" ||
                       mode_ == "ticker-only" || mode_ == "notes",
  ...
  if (!ticker_.empty()) {
    BOOST_ASSERT_MSG(
        !ticker_cache_file_name_.empty(),
        "You must provide a cache file when using ticker symbols.");
...
void CollectorApp::Do_Run_TickerFileLookup() {
  ticker_converter_.UseCacheFile(ticker_cache_file_name_);
  ticker_map_ =
      ticker_converter_.ConvertFileOfTickersToCIKs(ticker_list_file_name_);
...
void CollectorApp::Do_Run_DailyIndexFiles() {
  ...
  DailyIndexFileRetriever idxFileRet{HTTPS_host_, HTTPS_port_,
                                     "/Archives/edgar/daily-index"};

  Do_TickerMap_Setup();

  if (begin_date_ == end_date_) {
    auto remote_daily_index_file_name =
        idxFileRet.FindRemoteIndexFileNameNearestDate(this->begin_date_);
    auto local_daily_index_file_name =
        idxFileRet.HierarchicalCopyRemoteIndexFileTo(
            remote_daily_index_file_name, this->local_index_file_directory_,
            replace_index_files_);

    if (!index_only_) {
      FormFileRetriever form_file_getter{HTTPS_host_, HTTPS_port_};
      decltype(auto) form_file_list = form_file_getter.FindFilesForForms(
          form_list_, local_daily_index_file_name, ticker_map_);
      ...
      form_file_getter.ConcurrentlyRetrieveSpecifiedFiles(
          form_file_list, this->local_form_file_directory_, max_at_a_time_,
          replace_form_files_);
    }
  } else {
    auto remote_daily_index_file_list =
        idxFileRet.FindRemoteIndexFileNamesForDateRange(begin_date_, end_date_);
    auto local_daily_index_file_list =
        idxFileRet.ConcurrentlyHierarchicalCopyIndexFilesForDateRangeTo(
            remote_daily_index_file_list, local_index_file_directory_,
            max_at_a_time_, replace_index_files_);

    if (!index_only_) {
      FormFileRetriever form_file_getter{HTTPS_host_, HTTPS_port_};
      decltype(auto) form_file_list = form_file_getter.FindFilesForForms(
          form_list_, local_daily_index_file_list, ticker_map_);
      ...
      form_file_getter.ConcurrentlyRetrieveSpecifiedFiles(
          form_file_list, local_form_file_directory_, max_at_a_time_,
          replace_form_files_);
    }
  }
...
void CollectorApp::Do_Run_QuarterlyIndexFiles() {
  ...
  QuarterlyIndexFileRetriever idxFileRet{HTTPS_host_, HTTPS_port_,
                                         "/Archives/edgar/full-index"};

  if (begin_date_ == end_date_) {
    auto remote_quarterly_index_file_name =
        idxFileRet.MakeQuarterlyIndexPathName(begin_date_);
    auto local_quarterly_index_file_name =
        idxFileRet.HierarchicalCopyRemoteIndexFileTo(
            remote_quarterly_index_file_name, this->local_index_file_directory_,
            replace_index_files_);

    if (!index_only_) {
      FormFileRetriever form_file_getter{HTTPS_host_, HTTPS_port_};
      decltype(auto) form_file_list = form_file_getter.FindFilesForForms(
          form_list_, local_quarterly_index_file_name, ticker_map_);
    ...
    auto remote_index_file_list =
        idxFileRet.MakeIndexFileNamesForDateRange(begin_date_, end_date_);
    auto local_index_file_list =
        idxFileRet.ConcurrentlyHierarchicalCopyIndexFilesForDateRangeTo(
            remote_index_file_list, local_index_file_directory_, max_at_a_time_,
            replace_index_files_);

    if (!index_only_) {
      FormFileRetriever form_file_getter{HTTPS_host_, HTTPS_port_};
      decltype(auto) form_file_list = form_file_getter.FindFilesForForms(
          form_list_, local_index_file_list, ticker_map_);
